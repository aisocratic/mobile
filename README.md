...
9. **`public.users` is not the auth roster, and that breaks DM addressing server-side.** Nothing creates a `users` row on signup, and the same human exists as several rows (old sign-ups, Telegram imports, second emails) with no linking key. Consequences, verified against production: a person can be a live, registered chat member whose auth uid has *no* `users` row — so the People tab literally cannot show their reachable identity — while their same-named duplicate rows resolve to derived keys nobody holds, which the relay accepts wraps for without complaint. The app now mitigates what it can (same-name entries without a registered chat identity are hidden when a registered one exists, incoming first-contact DMs create the thread instead of being dropped, and recipient keys are re-resolved from the directory on every conversation open), but the cure is server-side: create/link a `public.users` row per auth account (keyed by `auth.uid`, matched on email for existing rows) and merge the duplicates.
